...
# ============================================================================
# Probe Installer
# Installs the Probe architecture framework to ~/.claude/probe
# ============================================================================
...
# --- Configuration ---
readonly REPO_URL="https://github.com/daviguides/probe.git"
readonly TMP_DIR="/tmp/probe-$$"
readonly CLAUDE_DIR="$HOME/.claude"
readonly TARGET_DIR="$CLAUDE_DIR/probe"
readonly SOURCE_SUBDIR="probe"

# --- Marketplace Configuration ---
readonly MARKETPLACE_URL="https://github.com/daviguides/claude-marketplace.git"
readonly PLUGIN_IDENTIFIER="probe@daviguides"
...
clone_repository() {
  printf "%b\n" "${BLUE}Cloning Probe repository...${NC}"
...
validate_source_structure() {
  ...
  if [ ! -d "$src_subdir" ]; then
    ...
    printf "%s\n" "Repository structure may have changed. Verify 'probe/' exists at repo root."
    exit 1
  fi
}
...
install() {
  printf "%b\n" "${BLUE}Installing Probe to $TARGET_DIR...${NC}"
  ...
  printf "%b\n\n" "${GREEN}✓ Probe installed successfully!${NC}"
}
...
print_next_steps() {
  ...
  printf "%b\n\n" "${BLUE}Next steps:${NC}"
  printf "%s\n" "1. Use /load-probe-context command to load Probe context when needed"
  printf "%s\n" "2. Or reference specific specs/context files in your prompts"
  printf "%s\n" "3. Docs: https://github.com/daviguides/Probe"
}
